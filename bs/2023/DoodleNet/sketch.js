...
function setup() {
	...
	let button = createButton('消去');
	//button.position(7, 200);
	button.mousePressed(clearCanvas);
	// Create 'label' and 'confidence' div to hold results
	label = createDiv('推定: ...');
	confidence = createDiv('信頼度: ...');
}
...
function gotResult(error, results) {
	...
	label.html('推定: ' + results[0].label);
	confidence.html('信頼度: ' + nf(results[0].confidence, 0, 2)); // Round the confidence to 0.01
}
